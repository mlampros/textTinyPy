...
std::vector<std::string> big_files::res_TOKEN(std::string x, std::vector<std::string> language, std::string language_spec, std::string LOCALE_UTF, bool FLAG_path, std::string read_file_delimiter, 
                                             ...
                                             int stemmer_batches, int threads, bool verbose, bool save_2single_file, std::string path_extend, std::string vocabulary_path) {
  TOKEN t(x);
...
std::vector<std::string> big_files::res_token_vector(std::vector<std::string> &VEC, std::vector<std::string> language, std::string language_spec, std::string LOCALE_UTF, 
  ...
  for (unsigned long long f = 0; f < VEC.size(); f++) {
    ...
    std::vector<std::string> tmp_vec = res_TOKEN(VEC[f], language, language_spec, LOCALE_UTF, false, "\t", max_num_char, remove_char, cpp_to_lower, cpp_to_upper, cpp_remove_punctuation,
                                                 ...
                                                 stemmer_gamma, stemmer_truncate, stemmer_batches, 1, false, false, "output_token.txt", vocabulary_path);
